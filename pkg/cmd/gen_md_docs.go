...
// genMdDocsCmd represents the docs generation command
var genMdDocsCmd = &cobra.Command{
	Use:    "gen-md-docs",
...
func fixContent(dst string) error {
	also := "### SEE ALSO"
	...
	if err != nil {
		return fmt.Errorf("failed reading data from file: %w", err)
	}
	...
	wrapped := fmt.Sprintf("```\n%s\n```\n", rootCmd.Long)
	final := strings.Replace(string(data), rootCmd.Long, wrapped, 1)
